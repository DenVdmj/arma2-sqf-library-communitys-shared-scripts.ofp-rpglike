...
funcOpenConversationDialog = {
    [
        ...
        ],
        {
            ...
            _callUserCodeIntoNPCScope = {
                // защитим все наши переменные (включа€ внутренние переменные диалоговой системы),
                ...
                // его область данных
                _npcRecord = arg(2);
                // его "пам€ть", то есть область данных, которой пользуетс€ писатель диалогов
                _npcUserScope = [_npcRecord, "#"] call funcStorageGet;

                // функи дл€ писател€ диалогов
                ...
                // записывает переменную в пам€ть непис€
                _setVar = {
                    [_npcUserScope, arg(0), arg(1)] call funcStorageSet
                };
                // читает переменную из пам€ти непис€
                _getVar = {
                    [_npcUserScope, arg(0), arg(1)] call funcStorageGet
                };
                // удал€ет переменные из пам€ти непис€
                _delVar = {
                    [_npcUserScope, _this] call funcStorageDel
                };
                // устанавливает метку-флаг в пам€ть непис€, например ("непись обиделс€" call _addFlag)
                _addFlag = {
                    [_npcUserScope, _this, true] call funcStorageSet
                };
                // удал€ет метку-флаг из пам€ти непис€, например ("непись обиделс€" call _delFlag)
                _delFlag = {
                    [_npcUserScope, [_this]] call funcStorageDel
                };
                // провер€ет наличие метки-флага в пам€ти непис€, например ("непись обиделс€" call _isFlag)
                _isFlag = {
                    [_npcUserScope, _this, false] call funcStorageGet
                };
                // добавл€ет к имени файла путь до директории с файлами диалогов текущего NPC
                _thisPath = {
